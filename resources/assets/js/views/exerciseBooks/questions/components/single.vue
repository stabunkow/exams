<template>
	<div class="container" @change="$emit('change', currentValue)">
		...
		<div class="pb-1"><span class="label label-default">单选题</span></div>
		<label class="row" v-for="(item, index) in options" :key="item">
			<div class="col-1">
				<input type="radio" :value="translateOption(index)" v-model="currentValue" :disabled="hasDone"/>
				<span class="radio-core">{{translateOption(index)}}</span>
			</div>
